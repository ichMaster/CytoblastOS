...
1. **`/generate-issues <phase>`** — decompose ROADMAP §`A.B` into
   `specification/roadmap/implementation/v{A.B}-issues.md`.
2. **`/upload-issues <file>`** — push that file to GitHub as `CYTO-xxx` issues
...
- `/run-phase 0.2` — run v0.2 (first generated dashboard) through generate → upload → execute
- `/run-phase v1.2` — run v1.2 (OS adapter contract + macOS adapter)
...
1. **Normalize** the phase to `vA.B` (e.g. `0.2` → `v0.2`). Derive:
   - `n` = the major (roadmap version), e.g. `v0.2` → `0`.
   - **Label** = `v{n}::version:{n}` (e.g. `v0::version:0`) — what
     `/execute-issues` filters on.
   - **Issues file** = `specification/roadmap/implementation/v{A.B}-issues.md`.
2. **Preflight checks** (fail fast — report and stop if any fails):
   ...
   - Read [specification/ROADMAP.md](../../../specification/ROADMAP.md) §`A.B` —
     confirm the phase exists and read its Goal/Tasks/DoD/Tests.
   - Confirm the phase's `Depends on:` line is satisfied — the prior version's
     phases are released, not merely written.
...
1. **Generate** — invoke `generate-issues` with `<phase>`.
   - If `v{A.B}-issues.md` **already exists**: this is a fork — pause and ask
     ...
     silently.
   - Success criterion: `specification/roadmap/implementation/v{A.B}-issues.md`
     exists and parses (has an Issues Summary Table with `CYTO-xxx` rows).
...
2. **Upload** — invoke `upload-issues` with the issues file
   ...
   - Success criterion: every `CYTO-xxx` in the issues file maps to an open
...
- An issue would need a destructive operation on the working machine rather than
  the disposable VM. Stop — that is never resolved by guessing.
- A sub-skill itself asks a question you can't answer from the phase's spec
  (e.g. an ambiguous task, or an entry in ROADMAP §Open decisions that this
  phase forces). Surface it rather than guessing.

### Step 4: Final report

After the chain completes (or aborts), summarize the whole run:

- **Stage 1:** issues file path + issue count + `CYTO-xxx` id range.
- **Stage 2:** GitHub issues created (CYTO-xxx → #), report path.
