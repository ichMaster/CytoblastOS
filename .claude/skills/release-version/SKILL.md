...
- `/release-version 1.2.0 macOS adapter + contract suite; denied/unsupported as results` -- bump with provided changelog items
...
phase within that version (`v1.2`→B=2), `C` = post-release fix on that phase.
So roadmap phase `vA.B` → semver `A.B.0`; a fix after it bumps `C` (e.g. v1.2 →
`1.2.0`, a follow-up fix → `1.2.1`). Releases are cut per phase — CytoblastOS
builds one phase per release and the user launches the next
([specification/ROADMAP.md](../../../specification/ROADMAP.md) §Versioning).
**Never change the version without explicit user confirmation.**
...
5. Confirm the phase is actually complete: its issues closed and its execution
   report (`specification/roadmap/implementation/v{A.B}-execution-report.md`)
   records pytest green + ruff clean. An incomplete phase is not released.
...
3. Summarize the changes into concise bullet points (group related commits;
   reference the ROADMAP phase `vA.B` and the `CYTO-xxx` ids where relevant)
...
3. **`RELEASE.txt`** (create if it doesn't exist): prepend a new version block at
   the top (after any header):
...
- **Don't modify source files.** This skill only touches version metadata (VERSION, README.md, RELEASE.txt), never core/adapter/module/policy code.
...
- **Never release an incomplete phase.** Failed or skipped issues mean no release — say so and stop.
